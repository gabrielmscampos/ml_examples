...
service_name="sklearn-v2-example"
s3_model_dir="s3://test-bucket/sklearn-v2-example"
storage_uri="\"s3://test-bucket/sklearn-v2-example\""
host_model_path="$tmp_kubeconfigs_path/sklearn-v2-example"

mkdir -p "$host_model_path"

cp ../../../models/scikit_custom_regressor/model.joblib $host_model_path/model.joblib
cp ../../mlserver/scikit_custom_regressor/my_handler.py $host_model_path/my_handler.py
cp ../../mlserver/scikit_custom_regressor/my_model.py $host_model_path/my_model.py
...
sed -e "s/{{ inference_service_resource_name }}/$service_name/g" \
    ...
    $templates_path/mlserver/sklearn_v2.yaml \
    > $tmp_kubeconfigs_path/sklearn-v2-isvc.yaml
...
deploy_inferenceservice_http $kube_api_url $cacert $cert $key "default" "$tmp_kubeconfigs_path/sklearn-v2-isvc.yaml" "$service_name"
...
if [ "$DELETE_AFTER_TESTING" == "true" ]; then
  ...
  kubectl delete -f $tmp_kubeconfigs_path/sklearn-v2-isvc.yaml
else
