...
service_name="xgboost-example"
s3_model_dir="s3://test-bucket/xgboost-example"
storage_uri="\"s3://test-bucket/xgboost-example\""
host_model_path="$tmp_kubeconfigs_path/xgboost-example"

mkdir -p "$host_model_path"

cp ../../../models/xgb_regressor/0001.ubj $host_model_path/0001.ubj
cp ../../xgboost_regressor/my_handler.py $host_model_path/my_handler.py
...
sed -e "s/{{ inference_service_resource_name }}/$service_name/g" \
    ...
    $templates_path/xgboost.yaml \
    > $tmp_kubeconfigs_path/xgboost-isvc.yaml

deploy_service "default" "$tmp_kubeconfigs_path/xgboost-isvc.yaml" "$service_name"
