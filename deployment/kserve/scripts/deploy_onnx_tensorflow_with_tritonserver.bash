...
service_name="onnx-tensorflow-tritonserver-example"
s3_model_dir="s3://test-bucket/onnx-tensorflow-tritonserver-example/my_model"
storage_uri="\"s3://test-bucket/onnx-tensorflow-tritonserver-example\""
host_model_path="$tmp_kubeconfigs_path/onnx-tensorflow-tritonserver-example/my_model"

mkdir -p "$host_model_path"
mkdir -p "$host_model_path/1"

cp ../../../models/tensorflow_as_onnx/model.onnx $host_model_path/1/model.onnx
...
sed -e "s/{{ inference_service_resource_name }}/$service_name/g" \
    ...
    > $tmp_kubeconfigs_path/onnx-tensorflow-tritonserver-example-isvc.yaml
...
deploy_inferenceservice_http $kube_api_url $cacert $cert $key "default" "$tmp_kubeconfigs_path/onnx-tensorflow-tritonserver-example-isvc.yaml" "$service_name"
...
jq '
  ...
  .outputs[].name |= sub("reconstructed"; "Identity:0") |
  .outputs[].name |= sub("avg_mse"; "Identity_1:0")
...
if [ "$DELETE_AFTER_TESTING" == "true" ]; then
  ...
  kubectl delete -f $tmp_kubeconfigs_path/onnx-tensorflow-tritonserver-example-isvc.yaml
else
